...
export async function ParseCoordinatorAsync(serializedObject, options) {
    ...
    await Promise.all(serializedObject._flowGraphs?.map(async (serializedGraph) => ParseFlowGraphAsync(serializedGraph, { coordinator, valueParseFunction, pathConverter: options.pathConverter })));
    return coordinator;
}
/**
